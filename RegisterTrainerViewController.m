...
@interface RegisterTrainerViewController ()
...
@implementation RegisterTrainerViewController
...
- (void)keyboardDidShow: (NSNotification *) notif
{
    ...
    float keyWidth = keyboardFrameBeginRect.size.width/3.0f;
    float keyHeight = keyboardFrameBeginRect.size.height/4.0f;
    
    _MButton = [UIButton buttonWithType:UIButtonTypeCustom];
    UIView *btn = [[UIView alloc] initWithFrame:CGRectMake(0,50, 50, 10)];
    btn.backgroundColor=[UIColor greenColor];
    _numberPad.inputAccessoryView=btn;
    /*_MButton.frame = CGRectMake(0,self.view.frame.size.height-keyHeight, keyWidth, keyHeight);
    _MButton.backgroundColor = [UIColor blueColor];
    [_MButton setTitle:@"M" forState:UIControlStateNormal];
    [_MButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_MButton setNeedsLayout];
    [_MButton addTarget:self
                 action:@selector(MButtonAction)
       forControlEvents:UIControlEventTouchUpInside];
    [_numberPad.superview addSubview:_MButton];
...
-(void)getTrainers
{
    ...
    if(object != nil){
        _passes = [NSMutableArray arrayWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:@"trainers"]];
            ...
            if(![responseString isEqualToString:@""])
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([self checkDuplicates:responseString]){
                        ...
                        [[NSUserDefaults standardUserDefaults] setObject:_passes forKey:@"trainers"];
                    }
